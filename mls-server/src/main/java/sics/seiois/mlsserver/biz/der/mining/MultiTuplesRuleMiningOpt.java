...
public class MultiTuplesRuleMiningOpt {
    private static Logger logger = LoggerFactory.getLogger(MultiTuplesRuleMiningOpt.class);
    ...
    public MultiTuplesRuleMiningOpt(int max_num_tuples, long maxOneRelationNum, long allCount, double sampleRatioForTupleCov,
                                    Diversity diversity,
    ...
    public MultiTuplesRuleMiningOpt(int max_num_tuples, InputLight inputLight, long support, float confidence,
                                    ...
                                    int index_null_string, int index_null_double, int index_null_long) {
        this(max_num_tuples, inputLight, support, confidence, maxOneRelationNum, allCount, tupleNumberRelations);
...
//    }
    ...
    public Map<TIntArrayList, List<Integer>> createHashMap(TIntArrayList list, List<Predicate> current, int pid, boolean isLeft, boolean same_pid) {
        ...
        /*
        // Note that, this is only for t0.A=t1.A with same predicate, bi-variable REEs.
        if (same_pid) {
            logger.info("before, hashMap size: {}", hashMap.size());
            ArrayList<TIntArrayList> remove_keys = new ArrayList<>();
            for (Map.Entry<TIntArrayList, List<Integer>> entry : hashMap.entrySet()) {
                if (entry.getValue().size() <= 1) {
                    remove_keys.add(entry.getKey());
                }
            }

            for (TIntArrayList rm_key : remove_keys) {
                hashMap.remove(rm_key);
            }
            logger.info("after, hashMap size: {}", hashMap.size());
        }
        */
...
//                counts[index] = sameSize;
    ...
    public List<Message> validationMap1(WorkUnits unitSet, Predicate pBegin, ArrayList<REE> partial_solution) {
        ArrayList<Message> messages = new ArrayList<>();
        List<WorkUnit> units = unitSet.getUnits();
        int[] pids = unitSet.getPids();
...
//                    rhsList.add(p);
//                }
//                Message message = new Message(currentList, rhsList, 0, 0, 0,
//                        new Long[0], new ArrayList<>(), new ArrayList<>());
//                // add into a list of messages
//                messages.add(message);
//            }
//            return messages;
...
//        logger.info("#### finish rightMap...");

        /*
        boolean num_equals = true;
        for (Map.Entry<TIntArrayList, List<Integer>> entry : leftMap.entrySet()) {
            if (!rightMap.containsKey(entry.getKey())) {
                num_equals = false;
                break;
            }
            int num_1 = entry.getValue().size();
            int num_2 = rightMap.get(entry.getKey()).size();
            if (num_1 != num_2) {
                num_equals = false;
                break;
            }
        }
        logger.info("#### number of leftMap and rightMap equals: {}", num_equals);
        */

//        logger.info("#### currentSetB: {}, constantPs: {}", currentSet, constantPs);
        for (Map.Entry<TIntArrayList, List<Integer>> entry : leftMap.entrySet()) {
...
//                logger.info("#### key: {}", key);
                this.updateHyperCubeMap1(hyperCubes, unitSet, currentSet, entry.getValue(), constantPs, pids, key, 0);
//                logger.info("####  updateHyperCubeMap1 - 1");
                this.updateHyperCubeMap1(hyperCubes, unitSet, currentSet, rightMap.get(key), constantPs, pids, key, 1);
//                logger.info("####  updateHyperCubeMap1 - 2");
            }
        }
//        logger.info("#### finish updateHyperCubeMap1!!!!!!!!!!");
        ...
        for (int index = 0; index < units.size(); index++) {
            ...
            HyperCube hyperCube = hyperCubes.get(index);
//            logger.info("#### X: {},\t Y: {}", currentList, currentList);
            hyperCube.getStatistic_new(ifSame, pids, this.diversity, this.maxOneRelationNum, partial_solution, this.sampleRatioForTupleCov);
//            logger.info("#### supportXCP0: {}, supportXCP1: {}", hyperCube.getSupportXCP0(), hyperCube.getSupportXCP1());
            /*
            if (pids[pBegin.getIndex1()] < pids[pBegin.getIndex2()]) {
                // hyperCube.getStatistic(false);
            ...
            // collect message
            Message message = new Message(currentList, rhsList, hyperCube.getSupportX(), hyperCube.getSupportXCP0(), hyperCube.getSupportXCP1(),
                    hyperCube.getMarginalTupleCoverage(), hyperCube.getTupleIDsCons_t0(), hyperCube.getTupleIDsCons_t1());
            message.updateAllCurrentRHSsSupport(hyperCube.getSuppRHSs(), rhsList);
            messages.add(message);
        }
//        logger.info("#### finish messages!!!!!!!!!!");
    ...
    /*
        ...
        currentSetB: not contain the constant predicates and the same predicates
     */
    private List<HyperCube> updateHyperCubeMap1(List<HyperCube> hyperCubes, WorkUnits unitSet, ArrayList<Predicate> currentSetB,
        ...
        // initialize all work units (Same LHS, different RHS)
        for (int index = 0; index < unitSet.getUnits().size(); index++) {
            WorkUnit unit = unitSet.getUnits().get(index);
            ArrayList<Predicate> rhs = new ArrayList<>();
            for (Predicate p : unit.getRHSs()) {
...
//        int countLine = 0;
        for (int lid = 0; lid < lines.size(); lid++) {
            int line = lines.get(lid); ////////////////// the id of tuples that satisfy the rule //////////////////
            //遍历常数谓词, filter
...
//            }
            ...
            for (int ppid = 0; ppid < currentSetB.size(); ppid++) {
                // 遍历所有谓词, add value for the rest predicate
                Predicate currp = currentSetB.get(ppid);
                int v = 0;
                if (indexUsed == 0) {
                    v = currp.getOperand1().getColumnLight().getValueInt(pids[currp.getIndex1()], line);
            ...
            for (int index = 0; index < unitSet.getUnits().size(); index++) { // index: the index of WorkUnit, i.e., RHS
                WorkUnit unit = unitSet.getUnits().get(index);
                if(ifConst[index]) {
                    // the support of LHS
                    if (indexUsed == 0) {
                        ...
                        if (rhs_.isConstant()) {
                            if (rhs_.getIndex1() == indexUsed) {
                                ...
                                if (v == rhs_.getConstantInt().intValue()) {
                                    ...
                                    // record the tuple ids
                                    if (this.diversity.getCandidate_functions().contains("tuple_coverage")) {
                                        // int offset = rhs_.getOperand1().getColumnLight().getTidsIntervals().get(pids[rhs_.getIndex1()]).getLeft();
                                        // int real_line = line + offset;
                                        hyperCubes.get(index).addTupleIDsCons(rhsID, pids[rhs_.getIndex1()], line, indexUsed, this.diversity); // record the tuple ids
                                    }
                                }
                            }
                        } else {
                            if (indexUsed == 0) {
                                int v = rhs_.getOperand1().getColumnLight().getValueInt(pids[rhs_.getIndex1()], line);
                                if (this.diversity.getCandidate_functions().contains("tuple_coverage")) {
                                    hyperCubes.get(index).addTupleT0(xValues[index], v, rhsID, pids[rhs_.getIndex1()], line, this.diversity); // record the tuple ids
                                } else {
                                    hyperCubes.get(index).addTupleT0(xValues[index], v, rhsID, line);
                                }
                            } else {
                                int v = rhs_.getOperand2().getColumnLight().getValueInt(pids[rhs_.getIndex2()], line);
                                if (this.diversity.getCandidate_functions().contains("tuple_coverage")) {
                                    hyperCubes.get(index).addTupleT1(xValues[index], v, rhsID, pids[rhs_.getIndex2()], line, this.diversity); // record the tuple ids
                                } else {
                                    hyperCubes.get(index).addTupleT1(xValues[index], v, rhsID, line);
                                }
                            }
...
//                countLine++;
    ...
    public List<Message> computeAttributeNonoverlap(WorkUnits unitSet) {
        ArrayList<Message> messages = new ArrayList<>();
        List<WorkUnit> units = unitSet.getUnits();
        int[] pids = unitSet.getPids();

        HashSet<String> covered_attrs_by_Sigma = this.diversity.getCoveredAttributes();

        for (int index = 0; index < units.size(); index++) {
            WorkUnit unit = units.get(index);
            HashSet<String> attrs_in_X = new HashSet<>();

            ArrayList<Predicate> currentList = new ArrayList<>();
            for (Predicate p : unit.getCurrrent()) {
                currentList.add(p);
                attrs_in_X.add("X_" + p.getOperand1().getColumnLight().getName());
                if (!p.isConstant()) {
                    attrs_in_X.add("X_" + p.getOperand2().getColumnLight().getName());
                }
            }

            // covered attributes intersection
            HashSet<String> result = new HashSet<>();
            HashMap<Integer, Integer> attr_nonoverlap = new HashMap<>();
            ...
            for (Predicate p : unit.getRHSs()) {
                ...
                // copy attrs in X
                HashSet<String> attrs_current = new HashSet<>();
                attrs_current.addAll(attrs_in_X);
                // add attrs in RHS
                attrs_current.add("Y_" + p.getOperand1().getColumnLight().getName());
                if (!p.isConstant()) {
                    attrs_current.add("Y_" + p.getOperand2().getColumnLight().getName());
                }
                // intersection with attrs in Sigma
                result.clear();
                result.addAll(covered_attrs_by_Sigma);
                result.retainAll(attrs_current);
                attr_nonoverlap.put(PredicateSet.getIndex(p), -result.size());
            }

            Message message = new Message(currentList, rhsList, attr_nonoverlap, "attr_nonoverlap");
            messages.add(message);
        }
        return messages;
    }
    ...
    public List<Message> computePredicateNonoverlap(WorkUnits unitSet) {
        ArrayList<Message> messages = new ArrayList<>();
        List<WorkUnit> units = unitSet.getUnits();
        int[] pids = unitSet.getPids();

        HashSet<Predicate> covered_pred_by_Sigma = this.diversity.getCoveredPredicates();

        for (int index = 0; index < units.size(); index++) {
            WorkUnit unit = units.get(index);
            HashSet<Predicate> pred_in_X = new HashSet<>();
            ...
            HashMap<Integer, Integer> pred_nonoverlap = new HashMap<>();
            ...
            for (Predicate p : unit.getRHSs()) {
                ...
                // copy predicate in X
                HashSet<Predicate> pred_current = new HashSet<>();
                pred_current.addAll(pred_in_X);
                // add predicate in RHS
                pred_current.add(p);
                // intersection with predicate in Sigma
                result.clear();
                result.addAll(covered_pred_by_Sigma);
                result.retainAll(pred_current);
                pred_nonoverlap.put(PredicateSet.getIndex(p), -result.size());
            }

            Message message = new Message(currentList, rhsList, pred_nonoverlap, "pred_nonoverlap");
            messages.add(message);
        }
        return messages;
    }

    // partial_solution.size() > 0
    public List<Message> computeMinAttributeDistance(WorkUnits unitSet, ArrayList<REE> partial_solution) {
        ArrayList<Message> messages = new ArrayList<>();
        List<WorkUnit> units = unitSet.getUnits();
        int[] pids = unitSet.getPids();

        double min_distance_old = this.diversity.getMin_distance();
        ...
        for (int index = 0; index < units.size(); index++) {
            ...
            ArrayList<Predicate> rhsList = new ArrayList<>();
            HashMap<Integer, Double> attribute_distance = new HashMap<>();
            for (Predicate rhs : unit.getRHSs()) {
                rhsList.add(rhs);

                // the min distance between {\Sigma, ree}
                double min_distance_curr = min_distance_old;
                for (REE ree : partial_solution) {
                    double dis = ree.computeAttributeJaccardDistance(currentList, rhs);
                    if (dis < min_distance_curr) {
                        min_distance_curr = dis;
                    }
                }

                // marginal distance
                if (partial_solution.size() == 1) {  // min_distance_old is Double.POSITIVE_INFINITY
                    attribute_distance.put(PredicateSet.getIndex(rhs), min_distance_curr);
                } else {
                    attribute_distance.put(PredicateSet.getIndex(rhs), min_distance_curr - min_distance_old);
                }
            }

            Message message = new Message(currentList, rhsList, attribute_distance);
            messages.add(message);
        }
        return messages;
    }

    public List<Message> initialZeroDiversity(WorkUnits unitSet) {
        ArrayList<Message> messages = new ArrayList<>();
        List<WorkUnit> units = unitSet.getUnits();
        int[] pids = unitSet.getPids();
        ...
        for (int index = 0; index < units.size(); index++) {
            WorkUnit unit = units.get(index);
            PredicateSet currentList = unit.getCurrrent();
            PredicateSet rhsList = unit.getRHSs();

            Message message = new Message(currentList, rhsList);
            messages.add(message);
        }
        return messages;
    }
