...
echo -e "---------- PTopkDivMiner algorithm ----------"
...
then 
    ...
    ./run.sh  support=${supp} confidence=${conf} taskID=${task[${dataID}]} highSelectivityRatio=0 interestingness=1.5 skipEnum=false dataset=${task[${dataID}]} topK=${topK} round=1 maxTupleVariableNum=${tnum} ifPrune=true outputResultFile=${outputFile_ptopkminer} algOption="diversified" numOfProcessors=${processor} MLOption=1 ifClusterWorkunits=0 filterEnumNumber=${filterEnumNumber} lambda=${lambda} relevance=${relevanceMeasures} diversity=${diversityMeasures} sampleRatioForTupleCov=${sampleRatioForTupleCov} w_supp=${w_supp} w_conf=${w_conf} w_attr_non=${w_attr_non} w_pred_non=${w_pred_non} w_attr_dis=${w_attr_dis} w_tuple_cov=${w_tuple_cov} w_rel_model=${w_rel_model} predicateToIDFile=${predicateToIDFile} relevanceModelFile=${relevanceModelFile} MAX_X_LENGTH=${MAX_X_LENGTH}
