...
do
    ...
    do
        ...
        output_model_dir=${data_dir}${data_name}"/train/model_initializedByBert/"
        mkdir -p ${output_model_dir}
        all_predicates_file=${data_dir}${data_name}"/all_predicates.txt"

        python -u ${code_dir}relevanceFixedEmbeds_main.py -rules_file ${rules_file} -train_file ${train_file} -test_file ${test_file} -pretrained_matrix_file ${pretrained_matrix_file} -output_model_dir ${output_model_dir} -all_predicates_file ${all_predicates_file} -lr ${lr} -hidden_size ${hidden_size} -rees_embed_dim ${rees_embed_dim} -epochs ${epochs} -batch_size ${batch_size} -times ${times} -withBertInitialization 1
    done
done
