...
def processAllRulesByPredicates(rees, all_predicates_str):
    rees_lhs_final, rees_rhs_final = [], []
    for rule in rees:
        lhs, rhs = rule.split(LHS_TO_RHS_SYMBOL)[0].strip(), rule.split(LHS_TO_RHS_SYMBOL)[1].strip()
        pred_lhs = [e.strip() for e in lhs.split(":")[1].strip().split(LHS_DELIMITOR_SYMBOL)]
        # del(pred_lhs[0])
        # del(pred_lhs[0])  # remove relation atom
        pred_rhs = rhs.split(",")[0].strip()
        # rees_lhs_final.append(pred_lhs)
        # rees_rhs_final.append(pred_rhs)
        index_lhs = [all_predicates_str.index(e) for e in pred_lhs] + (MAX_LHS_PREDICATES - len(pred_lhs)) * [all_predicates_str.index(PADDING_VALUE)]
        index_rhs = [all_predicates_str.index(pred_rhs)] + (MAX_RHS_PREDICATES - len(pred_rhs)) * [all_predicates_str.index(PADDING_VALUE)]
        rees_lhs_final.append(index_lhs)
        rees_rhs_final.append(index_rhs)
    return rees_lhs_final, rees_rhs_final
